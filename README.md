...
현재 사내 LLM 시험용 Windows 배포본은 [0.1.0-internal.12 ZIP](artifacts/release/DiagramMaker-0.1.0-internal.12-win-x64.zip)과
[SHA-256](artifacts/release/DiagramMaker-0.1.0-internal.12-win-x64.zip.sha256)입니다.
기능별 검증 결과는 [자연어 복구·진단 검증 보고서](NATURAL_DIAGRAM_RECOVERY_REPORT.md), 사내 시험 절차는
...
코드/Git의 일부 의미 검토가 실패하면 통과한 설명은 **부분 완료 AI** 결과에 보존합니다. AI 항목 안에서 실패 상태를 확인하고, 페이지 설명에서 검토 사유·근거·보정 지시를 확인할 수 있습니다. **Code**는 별도 정적 원본을 유지합니다. Git 변경점 선택을 해제했다가 다시 선택하면 마지막 그룹 설정과 순서를 복원하며, 그룹 드롭다운에서 새 그룹을 만들어 바로 배정할 수 있습니다.

**구조 편집**에서 노드·관계·클래스 멤버·시퀀스 조건/메모를 더블클릭해 문구를 수정합니다. 실행 취소·다시 실행을 거쳐 새 리비전으로 저장할 수 있습니다. 보기 모드는 그림 위 좌클릭 드래그로 이동하며, 편집 모드는 빈 공간 또는 **Space+드래그**로 이동합니다.

합성 1,212행/40함수의 코드 3종·Git 3종 요청 수는 14→10회·28→20회이며 모든 결과 페이지와 의미 검토를 보존합니다. 실제 사내 모델의 처리 시간 개선은 별도 측정 대상입니다.
...
화면은 전체 누적값과 이번 실행의 완료·재사용·요청·실제 HTTP 전송·대기 시간을 구분합니다. **작업 진단 다운로드**의 v2 JSON에는 단계, 전송 여부, 토큰 한도/사용량, 시간과 오류 코드만 들어가며 코드·프롬프트·모델 응답·서버 주소는 포함하지 않습니다. 묶음 기본 한도는 입력 48,000토큰, 생성 출력 8,000토큰, 검토 출력은 동봉 정책 예제에서 2,000토큰이며 실제 설정값과 서버 상한을 존중합니다. Thinking은 기존 출력 예약을 사용합니다. 합성 요청 수와 사내 실모델의 5분 목표는 별도로 확인합니다.
...
최신 Windows 시험 패키지는 [0.1.0-internal.12 ZIP](artifacts/release/DiagramMaker-0.1.0-internal.12-win-x64.zip)과 [SHA-256](artifacts/release/DiagramMaker-0.1.0-internal.12-win-x64.zip.sha256)입니다. 기능별 변경 사항과 검증 결과는 [최신 검증 보고서](NATURAL_DIAGRAM_RECOVERY_REPORT.md) 및 [진행 기록](CODE_BLOCK_DIAGRAM_PROGRESS.md)에 기록했습니다.
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\build-offline-win-x64.ps1 -Version '0.1.0-internal.12'
node scripts/smoke-offline-preview.mjs artifacts/stage/DiagramMaker-0.1.0-internal.12-win-x64
node scripts/smoke-code-block-ui.mjs artifacts/stage/DiagramMaker-0.1.0-internal.12-win-x64
node scripts/smoke-packaged-llm.mjs artifacts/stage/DiagramMaker-0.1.0-internal.12-win-x64
node scripts/smoke-packaged-llm.mjs artifacts/stage/DiagramMaker-0.1.0-internal.12-win-x64 --basic
node scripts/smoke-windows-launchers.mjs artifacts/stage/DiagramMaker-0.1.0-internal.12-win-x64
```
...
**LLM 점검** 메뉴와 `test-llm.cmd`는 고정된 합성 데이터만 사용합니다. **코드 생성 검사**와 `test-code-diagram.cmd`는 C#/C++의 실제 파서·공유 생성·검토를 Thinking OFF로 확인하고 비민감 설정·오류 진단을 텍스트로 제공합니다. **자연어 생성 검사**와 `test-natural-diagram.cmd`는 짧은 승인 흐름과 표·공통 인터락 네 형식을 Thinking OFF로 검사합니다(관리자 전용, 최대 900초). 자연어 추출/검토는 각각 최대 3회로 분리 보정하며 전체 오류 목록과 JSON/텍스트 진단을 제공합니다. 보정 소진은 원인 확인 후 새 실행으로 안내합니다. 코드 블럭/Git 작업에도 텍스트 진단 다운로드가 있습니다. Git 다이어그램의 의미 설계에는 선택 변경과 관련 선언·제어 흐름·호출 근거 및 필요한 소스 조각을 전송합니다. 기존 내부 vLLM 연결·정확한 origin 검증·비밀 마스킹 정책을 그대로 사용하며 외부 모델로 자동 전환하지 않습니다.
...
이번 시험의 명시적 패키징 버전은 `0.1.0-internal.12`이며 출력은 `artifacts/release/DiagramMaker-0.1.0-internal.12-win-x64.zip`과 SHA-256 파일입니다. 기존 ZIP이 있으면 새 버전명을 지정해야 합니다. 실제 사내 LLM의 의미 품질은 사내 환경에서 별도로 확인해야 합니다.
