...
- Mermaid를 브라우저에서 지연 로드해 미리보기, 구조화된 노드·관계 편집 리비전, SVG/PNG 다운로드 제공
...
6. 결과의 **구조 편집**에서 노드와 관계를 추가·삭제·이름 변경·정렬하거나 방향을 바꿀 수 있습니다. 수정은 원본을 덮어쓰지 않고 새 리비전으로 저장되며 SVG/PNG로 내려받을 수 있습니다.
...
결과는 `artifacts/release/DiagramMaker-0.1.0-offline.13-win-x64.zip`과 SHA-256 파일입니다. 회사 정책상 GitHub Release 다운로드가 막힌 환경을 위해 이 두 파일은 소스 ZIP에도 포함되도록 관리합니다.
