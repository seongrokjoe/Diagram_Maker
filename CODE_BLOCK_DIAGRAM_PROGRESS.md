...
## 2026-09-23 생성 파이프라인 재개·검증·배포

- 2026-09-23 재개: 기존 미커밋 구현을 보존했다. 현재 .NET 428개 중 427개 통과, 1개는
  OneDrive 경로를 거부하는 기존 로컬 경로 테스트다. 웹 64개 통과, TypeScript 검사 통과;
  원본 폴더의 웹 빌드는 OneDrive 금지 정책에서 중단된다. `git diff --check` 통과.
  자연어 고정 검사에 사례/형식 선택 NDJSON 진행·완료 및 잘못된 선택의 HTTP 회귀를 추가했다.
  다음: 비동기화 검증 복사본에서 전체 verify와 API/UI 회귀를 실행하고 실패를 수정한다.
- 비동기화 복사본의 전체 verify에서 .NET 428/428, 작업자·웹·정책 테스트, 프런트엔드 빌드,
  API 두 모드 및 기본 공유 의미 검사가 통과했다. 60,000자 공유 검사와 후속 UI·실행기 검사 진행 중.
  다음: 전체 종료 결과 확인, 실패가 있으면 보완 후 재검증하고 새 패키지를 만든다.
- 전체 `verify.ps1` 최종 exit 0: .NET 428, 작업자 43, 웹 64, 정책 13, 시작 정책 11,
  API 두 모드, 공유 의미 기본/60,000자, SVG 6, Sequence 4, 코드·디자인 UI, 자연어 신뢰성
  합성 검사와 Windows UI 실행기 14개가 통과했다. 새 NDJSON 선택·진행·완료 API와 선택 UI도
  자연어 합성 검사에서 확인했다. 로그는 비동기화 복사본 `artifacts/internal15-validation/full-verify.log`.
  다음: internal.15 패키지 빌드와 ZIP/패키지 검사, 결과 문서 및 이전 ZIP 정리.
- internal.15 패키지 빌드 exit 0. ZIP 96,554,667바이트·1,832파일의 압축 내용과 stage를
  파일별 SHA-256으로 대조했으며 `.sha256` 값은
  `4021e89e1dcd9ce4ce3bbb631cc9eb065270a2f3208beef8c1edc9fbcbd6de46`이다.
  manifest는 기준 `802813a`와 dirty 상태를 기록한다. 다음: 패키지 검사 9개 종료 확인.
- 패키지 검사 7개 통과. 공유 60,000자 검사에서 코드 리비전 저장 버튼 대기 10초 시간 초과가
  1회 발생했다. 동일 소스 전체 검사와 패키지 기본 공유 검사는 통과했다. 첫 실패 로그와 결과 JSON을
  보존하고 통과한 7개는 재사용하여 해당 검사만 단독 재실행한다. 아직 패키지 검증 완료가 아니다.
- 60,000자 공유 검사가 단독 재실행에서 통과했고 Windows CMD 8개도 통과했다. 패키지 검사
  9개 명령의 최종 exit가 모두 0이다. 첫 실패와 재실행 결과를 함께 보존했다. ZIP/SHA를 원본
  `artifacts/release/`로 복사한 뒤 SHA-256을 재확인했다. 사용자 요청에 따라 이전 internal.13/
  internal.14 ZIP과 SHA 파일 4개를 제거했으며 pre-feature checkpoint는 보존했다.
...
- 사용자 승인: 고정 검사 실패를 반영한 시나리오 단위 생성, 근거 있는 검토, 단위별 공유 보정 예산,
  시나리오 연결 전용 수정, 상태 경계, 점검 진행/취소/마감 및 진단 개편을 구현한다.
- 시작 HEAD `802813a`, 작업 트리 깨끗함. 기존 checkpoint와 internal.13/internal.14 배포를 보존한다.
- 사내 증상: NaturalReviewInvalid 79, NaturalIntegrationScopeInvalid 11, NaturalScenarioInvalid 7,
  NaturalStateBoundaryInvalid 9 및 전체 검사 900초 예산 소진. 사내 모델 연결은 이 환경에 없다.
- 분석 기준선: 관련 테스트 86개와 후속 자연어 테스트 50개 통과. 구현 완료나 사내 해결 증거는 아니다.
- 다음: 계약/식별자/보정 상태 회귀 → 시나리오 생성 및 검토 → 코드/Git 검토 → 점검 API/UI → 전체 검증/패키지.
- 재개 구현: 형식별 시나리오 계약, 링크 ID의 프롬프트/스키마/응답 일치, 근거 기반 단일 문제 목록 검토,
  공유 보정 예산/재개 체크포인트, 시나리오 연결 전용 수정 및 상태 경계 검증을 구현했다.
  핵심 신규 회귀 12개와 API 빌드(경고/오류 0) 통과. 전체 완료를 뜻하지 않는다.
- 형식 간 검토의 지적 페이지 보정과 분할 시 공유 예산을 연결했고, 코드/Git 검토를 구체적 수정 지시와
  근거 ID로 바꿨다. 고정 검사의 NDJSON 진행 API, 사례/형식 선택, 취소, 남은 시간 및 원인/종료 진단을 추가했다.
- 첫 전체 .NET 실행은 363 통과/59 실패. 구형 합성 응답 계약·후보별 예산 기대값을 갱신하며,
  오류 포장 및 변경 없는 후보의 조기 중단 경로를 보완했다. 로컬 경로 테스트는 OneDrive 제한 영향이다.
  다음: 변경된 계약의 회귀 실패 해결, 취소/시간 제한·선택 보정 회귀, 비동기화 복사본 전체 verify와 패키지.
