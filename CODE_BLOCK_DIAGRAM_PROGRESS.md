...
### 2026-09-23 자연어 다이어그램 검증·보정 개선 시작

- 사용자 승인 계획에 따라 internal.15 깨끗한 작업 트리(`ffd19cf`)에서 시작했다. 실제 사내 모델은 GLM5.2로 알려졌고 Thinking은 꺼져 있다. 사내 비교 화면과 조건 변경 지적의 후보별 1회 확인 검토를 선택했다.
- 1단계: 시나리오 연결 검증을 빈 목록·중복 ID·알 수 없는/누락된 요구사항·질문 근거로 분리했다. 추출·연결 보정 응답에서 시나리오 원문 범위를 제거하고 승인 요구사항 근거로 서버가 계산하도록 바꿨다. 저장 계약은 유지한다.
- 변경된 추출 계약에 맞게 합성 LLM 응답을 수정했다. 관련 .NET 회귀 40개 통과. 실제 사내 LLM과 PostgreSQL은 아직 검증하지 않았다.
- 다음: 노드/연결 구조 검증의 대상·필드를 세분화하고, 상세 오류를 보정·진단에 연결한다. 현재 원본 저장소에서 좁은 테스트만 실행했으며 전체 검증과 패키징은 미실행이다.- 2단계: 흐름도·시퀀스·상태도·클래스 노드/연결의 종류·형태·요구사항 참조·가정 표시·조건 경로를 대상과 필드별로 보고한다. 공통 인터락과 상태 경계도 관련 요구사항/노드에 연결했다. 시나리오 보정에는 상세 오류 전체를 전달하고 복구 관찰 키를 범위별로 분리했다.
- 관련 .NET 회귀 55개 통과. 기존 단일 시나리오 오류 수를 기대하던 테스트는 새 세부 오류 검증으로 갱신했다.
- 다음: 의미 검토의 원문 인용 검증, 후보당 확인 검토, 비공개 비교 자료·화면을 구현한다. 실제 모델에서 지적한 조건이 실제 변경인지 오판인지는 아직 확인할 수 없다.- 3단계: 시나리오 의미 검토의 조건 변경 지적은 원문 범위에 존재하는 직접 인용과 유효한 대상/필드를 요구한다. 유효한 조건 변경 후보는 내용 보정 예산 안에서 한 번 더 확인하며, 후보별 체크포인트 재사용을 적용했다. 관련 .NET 회귀 40개 통과.
- 다음: 오류별 비공개 비교 스냅샷·소유자 API/UI와 메타데이터 진단을 연결한다. 확인 검토의 명시적 회귀 및 전체 검증은 아직 미실행이다.- 4단계: 오류 당시 근거·생성 요소·수정 지시를 마스킹한 비공개 비교 체크포인트를 저장하고, 실행 소유자 전용 비교 API/UI를 연결했다. 일반 진단 다운로드는 메타데이터만 유지한다. Buffer Handshake 시퀀스 고정 검사를 추가하고 프로토콜/생성기를 v8/v12로 올렸다.
- 자연어 관련 .NET 회귀 96개 통과. 추가된 상세 검증·비교 테스트 5개 통과. 웹 TypeScript 검사는 통과했으나 OneDrive 원본 경로에서 웹 빌드가 정책상 차단됐다. 전체 .NET 428개 중 기존 OneDrive 경로 관련 2개 실패가 있었으며 비동기화 사본에서 재검증해야 한다.
- 다음: 합성 API/UI 검사 스크립트를 새 계약에 맞춰 검증하고, 비동기화 사본에서 전체 verify 및 패키지 검증을 수행한다. 실제 사내 LLM 검증은 별도 대기다.
- 5단계 검증: 비동기화 사본에서 .NET 433개, Node 작업자 43개, 웹 빌드 및 API·코드 UI 검사를 통과했다. 공유 의미 검사의 고정 요청 수가 신규 조건 변경 확인 1회를 반영하지 않아 실패했다. 검사 기대값을 15/18회로 갱신하고 해당 검사를 재실행 중이다. 전체 검증과 패키지는 아직 미완료다.
- 다음: 공유 의미 검사를 통과시키고 전체 verify를 다시 완료한 뒤 internal.16 패키지와 배포 전 검사를 진행한다.

- 5단계 추가 수정: 자연어 API/UI 검사에서 마스킹된 원문 인용을 원문과 직접 비교해 NaturalReviewEvidenceInvalid로 거부하던 결함을 발견했다. 검토기는 원문과 모델에 실제 전달된 마스킹 표현을 모두 근거로 허용한다. 관련 .NET 회귀 테스트 5개와 자연어 합성 API/UI 검사가 통과했다. 3개 고정 사례로 늘어난 요약 행 수 검사도 갱신했다.
- 다음: 소스 동기화 확인 후 전체 verify 재실행, internal.16 패키지 빌드와 패키지 검사, 보고서·배포 정리·Git push를 완료한다.

- 5단계 완료: 비동기화 검증 사본에서 powershell -ExecutionPolicy Bypass -File .\scripts\verify.ps1 -UseInstalledDependencies 최종 exit 0. .NET 434개, Node 작업자 43개, 웹·정책·라이선스·API, 공유 의미 기본/60,000자, 자연어 합성 API/UI 21개 검사, Windows UI 실행기 14개가 통과했다. 로그는 검증 사본 rtifacts/internal16-full-verify-final.log에 있다. 사내 실제 LLM/PostgreSQL/egress 검사는 수행하지 못했다.
- 다음: internal.16 Windows ZIP을 빌드하고 9개 패키지 검사 및 ZIP 무결성을 확인한 뒤 기존 internal.15 ZIP/SHA를 정리한다.

- 6단계 패키지 검증: internal.16 Windows ZIP 빌드 exit 0. ZIP 96,569,807바이트·1,832파일, SHA-256 2c4379eb4710860fb0226cc520322af68dabfe039692c5fc1eaeae7297a3cbee. ZIP의 모든 파일을 stage와 개별 SHA-256 대조했고 일치했다. 미리보기, 코드·디자인 UI, 자연어 API/UI, LLM 기본·간편, 공유 의미 기본·60,000자, Windows 실행기 총 9개 패키지 검사가 통과했다. 사내 실제 GLM 5.2/PostgreSQL 검증은 미수행이다.
- 다음: ZIP/SHA와 검증 증적을 원본 artifacts로 옮기고 이전 internal.15 ZIP/SHA를 제거한 뒤 보고서·최종 검토·커밋·push를 진행한다.
