...
export async function checkDesignUi({ page, origin, fixture }) {
  ...
  try {
    ...
    await page.getByRole('button', { name: '그룹 추가', exact: true }).click();
    await candidateRows.nth(2).getByRole('checkbox').uncheck();
    await candidateRows.nth(3).getByRole('checkbox').uncheck();
    assert.equal(await page.locator('.group-card').count(), 1, 'a new empty draft survives unrelated removals');
    assert.ok(await page.getByRole('button', { name: '선택대로 다이어그램 생성', exact: true }).isDisabled());
    for (let i = 0; i < 4; i++) await candidateRows.nth(i).getByRole('checkbox').check();
    ...
    const resultTree = page.getByRole('tree', { name: '다이어그램 결과 트리' });
    await resultTree.locator('[aria-level="4"]').last().click();
    assert.equal(await resultTree.locator('[aria-selected="true"]').count(), 1);
