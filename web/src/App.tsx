    ...
    <main>
      {tab === "natural" && <section className="workspace-grid natural-workspace">
        ...
        <section className="panel preview">
          ...
          {selectedNatural?.designQuality && <details className="diagnostic-panel"><summary>{selectedNatural.designQuality.status === "Reviewed" ? "생성 원본 설계 검토 완료" : "검토 후 편집됨"} · 요구사항 {selectedNatural.designQuality.reviewedRequirementIds.length}개 · 설계 가정 {selectedNatural.designQuality.assumptionElementIds.length}개</summary>
            <p className="help">설계 가정은 그림에 표시됩니다. 수동 편집한 내용은 별도 검토가 필요합니다.</p>
            <ul>{naturalRecord?.requirements?.requirements.map(item => <li key={item.id}>{item.origin === "assumption" ? "설계 가정: " : "요구사항: "}{item.text}</li>)}</ul>
          </details>}
          {diagram && naturalRecord && selectedNatural ? <DiagramEditor artifact={diagram} downloadName={`natural-${diagram.type}`} reportError={reportError} zoomable canvasToolbar
            onSave={(input) => api.saveNaturalDiagramEdit(naturalRecord.id, selectedNatural.viewId, input)}
            onPreview={(input, signal) => api.previewNaturalDiagramEdit(naturalRecord.id, selectedNatural.viewId, input, signal)} /> : <EmptyState text="요청을 입력하면 결과가 여기에 표시됩니다." />}
