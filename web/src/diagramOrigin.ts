...
export type DiagramResultCounts = { aiCompleted: number; aiFailed: number; aiPending: number; codeCompleted: number; reusedAi?: number; aiPartial?: number };
export function originOf(artifact: DiagramArtifact): DiagramVariant {
  const explanation = artifact.explanation;
  return explanation?.status === "Semantic" || explanation?.status === "Incomplete" && (explanation.coverage?.verifiedUnits ?? 0) > 0 ||
    artifact.ir.provenance?.some(p => p.startsWith("natural")) ? "ai" : "code";
}
...
export function resultCountsText(c: DiagramResultCounts) {
  return `AI 완료 ${c.aiCompleted}개 · AI 부분 ${c.aiPartial ?? 0}개 · AI 실패 ${c.aiFailed}개 · 대기 ${c.aiPending}개 · Code 완료 ${c.codeCompleted}개${c.reusedAi ? ` · 이전 AI 유지 ${c.reusedAi}개` : ""}`;
}
