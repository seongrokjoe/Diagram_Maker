...
- Semantic checkpoints contain structured intermediate results and completed model responses, including rejected responses needed to resume a bounded repair without resetting its budget. They inherit the source run's storage access and deletion rules and are not included in diagnostics or summary endpoints. Repair requests may include the rejected model response as untrusted data sent only to the same configured LLM; rejected responses are not persisted as diagnostics or logged.
...
## Natural-language generation runs

Natural-language generation runs retain the owner's original prompt, UTF-16 requirement ranges, scenario assignments, intermediate pages and failure diagnostics in the configured application store. Run listing, retrieval, cancellation and resumption enforce owner authorization; revision/lease compare-and-swap rejects stale worker writes. LocalFile retains its single-instance deployment restriction. PostgreSQL adds a `natural_diagram_runs` table for this state; existing diagram records remain readable. Prompt and model response bodies must not be logged. Partly reviewed AI pages disclose failed units and preserve a separate static Code original; manual text edits do not claim renewed semantic review.
...
The natural diagram self-test also requires Admin and accepts no user prompt. It runs fixed synthetic approval and table/interlock cases through the production generation service, with Thinking disabled and an isolated disposable store. Its total budget respects the configured limit and cannot exceed 900 seconds. Natural schema compatibility retains local validation against the original schema; exported diagnostics contain fixed validation codes and schema field paths only.

Natural extraction wire contracts accept source-linked requirements only; provenance and quotes are assembled by the server after evidence validation. Semantic review still checks source fidelity. Review issue categories are allowlisted, require source references, and never export free-form correction instructions. Per-candidate review contract repair is bounded separately from extraction repair, with at most 12 logical completions per extraction unit and the existing whole-run time budget. Metadata diagnostics and copy summaries include counts and fixed categories only; unchanged rejected candidates do not trigger repeated semantic reviews.
